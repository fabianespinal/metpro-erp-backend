...
def update_client(client_id: int, client: ClientBase, current_user: dict = Depends(verify_token)):
    ...
    try:
        ...
        cursor.execute('''
            UPDATE clients 
            SET company_name = %s, contact_name = %s, email = %s, phone = %s, address = %s, tax_id = %s, notes = %s
            WHERE id = %s
        ''', (
            client.company_name,
            client.contact_name,
...
def get_quote(quote_id: str, current_user: dict = Depends(verify_token)):
    ...
    try:
        ...
        return {
            ...
            'status': quote['status'],
            'notes': quote['notes'],
            'included_charges': json.loads(quote['included_charges']),
            'items': items_list
        }
    finally:
        if conn:
...
def get_quote_pdf(quote_id: str, current_user: dict = Depends(verify_token)):
    ...
    try:
        ...
        if client.get('contact_name'):
            pdf.cell(0, 6, f'Contacto: {client["contact_name"]}', 0, 1)
        if client.get('email'):
            pdf.cell(0, 6, f'Email: {client["email"]}', 0, 1)
        if client.get('phone'):
            pdf.cell(0, 6, f'Telefono: {client["phone"]}', 0, 1)
        if client.get('address'):
            pdf.cell(0, 6, f'Direccion: {client["address"]}', 0, 1)
        if client.get('tax_id'):
            pdf.cell(0, 6, f'RNC: {client["tax_id"]}', 0, 1)
        pdf.ln(5)
...
def update_product(product_id: int, product: ProductBase, current_user: dict = Depends(verify_token)):
    ...
    try:
        ...
        cursor.execute('''
            UPDATE products SET name = %s, description = %s, unit_price = %s WHERE id = %s
